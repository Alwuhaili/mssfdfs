...
export const RoleAuthModal: React.FC<RoleAuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden font-arabic flex flex-col max-h-full">
        
        {/* Header Banner */}
        <div className="shrink-0 bg-gradient-to-r from-indigo-700 via-indigo-600 to-indigo-800 p-5 sm:p-6 text-white relative">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-3 pr-8 sm:pr-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white/10 border border-white/20 text-xl sm:text-2xl flex items-center justify-center shadow-inner shrink-0">
              {roleInfo.iconStr}
            </div>
            <div>
              <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-bold text-amber-300">
        ...
        {/* Form Content */}
        <form onSubmit={handleVerifyAndSwitch} className="overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-amber-50 border border-amber-200 p-3 rounded-2xl">
            ...
            <button
              ...
              className="w-full sm:w-auto px-3 py-2 sm:py-1.5 bg-amber-400 hover:bg-amber-500 text-slate-900 font-bold text-xs rounded-xl shadow-sm transition-all flex items-center justify-center gap-1 shrink-0"
            >
          ...
          {/* Footer Actions */}
          <div className="pt-3 pb-1 flex items-center justify-end gap-2 sm:gap-3 border-t border-slate-100 shrink-0 mt-auto sticky bottom-0 bg-white">
            <button
              type="button"
              onClick={onClose}
              className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-slate-300 text-slate-700 font-bold text-xs hover:bg-slate-100 transition-all"
            >
              {lang === 'ar' ? 'إلغاء' : 'Cancel'}
            </button>
            <button
              type="submit"
              className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md shadow-indigo-600/20 transition-all flex items-center gap-1.5 sm:gap-2"
            >
