...
export const TeacherDashboard: React.FC<{ activeTab: string }> = ({
  activeTab,
}) => {
  const {
    currentUser,
  ...
  // FirebaseAuthService has already verified this Firestore profile against Firebase UID.
  const verifiedTeacherProfile =
    currentUser?.role === "teacher" &&
    currentUser.authUid &&
    currentUser.teacherObj?.id &&
    currentUser.teacherObj.authUid === currentUser.authUid
      ? currentUser.teacherObj
      : null;

  // Synced teacher data may enrich the verified profile, but cannot establish identity.
  const syncedTeacher = verifiedTeacherProfile
    ? teachers.find((tech) =>
        (tech.authUid && tech.authUid === currentUser?.authUid) ||
        tech.id === verifiedTeacherProfile.id
      )
    : undefined;

  const activeTeacher = verifiedTeacherProfile
    ? {
        ...verifiedTeacherProfile,
        ...(syncedTeacher || {}),
        id: verifiedTeacherProfile.id,
  ...
  const [examSubject, setExamSubject] = useState(
    activeTeacher?.subject || "الفيزياء المتقدمة",
  );
  const [examGrade, setExamGrade] = useState<GradeLevel>(
    activeTeacher?.assignedGrades?.[0] || "الصف السادس العلمي",
  );
  ...
  const [resultFilterGrade, setResultFilterGrade] = useState<
    GradeLevel | "الكل"
  >("الكل");
  const [resultFilterSection, setResultFilterSection] =
  ...
  // Attendance State
  const [teacherAttTab, setTeacherAttTab] = useState<"record" | "review">(
    "record",
  );
  const [attendanceDate, setAttendanceDate] = useState(
    new Date().toISOString().split("T")[0],
  );
  const [attendanceGrade, setAttendanceGrade] = useState<GradeLevel>(
    activeTeacher?.assignedGrades?.[0] || "الصف السادس العلمي",
  );
  ...
  const [attendanceMap, setAttendanceMap] = useState<
    Record<string, "حاضرة" | "غائبة" | "متأخرة" | "مجازة">
  >({});
  const [dispatchReportModal, setDispatchReportModal] =
    useState<AttendanceDispatchReport | null>(null);
  const [isExportingPdf, setIsExportingPdf] = useState(false);

  // Lecture Upload State
  const [lecTitle, setLecTitle] = useState("");
  const [lecSubject, setLecSubject] = useState(
    activeTeacher?.subject || "الفيزياء المتقدمة",
  );
  const [lecGrade, setLecGrade] = useState<GradeLevel>(
    activeTeacher?.assignedGrades?.[0] || "الصف السادس العلمي",
  );
  const [lecType, setLecType] = useState<"video" | "pdf" | "doc" | "ppt">(
    "pdf",
  );
  ...
  const [teacherLectureGradeFilter, setTeacherLectureGradeFilter] =
    useState<string>("all");
  const [teacherLectureSubjectFilter, setTeacherLectureSubjectFilter] =
    useState<string>("all");
  const [teacherCategoryFilter, setTeacherCategoryFilter] =
    useState<string>("all");
  ...
  const teacherFilteredLectures = lectures.filter((lec) => {
    const matchesGrade =
      teacherLectureGradeFilter === "all" ||
      lec.gradeLevel === teacherLectureGradeFilter;
    ...
    const matchesSubject =
      ...
      (teacherLectureSubjectFilter === "الرياضيات" &&
        lec.subject.includes("الرياضيات")) ||
      (teacherLectureSubjectFilter === "الفيزياء" &&
        lec.subject.includes("الفيزياء")) ||
      (teacherLectureSubjectFilter === "علم الاحياء" &&
        (lec.subject.includes("الأحياء") || lec.subject.includes("الاحياء"))) ||
      (teacherLectureSubjectFilter === "الحاسوب" &&
        lec.subject.includes("الحاسوب")) ||
      (teacherLectureSubjectFilter === "اللغة الانجليزية" &&
        (lec.subject.includes("الانجليزية") ||
          lec.subject.includes("الإنكليزية") ||
          lec.subject.includes("الإنجليزية"))) ||
      (teacherLectureSubjectFilter === "التربية الاسلامية" &&
        (lec.subject.includes("الاسلامية") ||
          lec.subject.includes("الإسلامية"))) ||
      (teacherLectureSubjectFilter === "التربية الاخلاقية" &&
        (lec.subject.includes("الاخلاقية") ||
          lec.subject.includes("الأخلاقية")));
    const matchesSearch =
  ...
  // Fail closed: no verified teacher profile = no exam creation.
  if (!activeTeacher) {
    alert(
      lang === "ar"
        ? "تعذر التحقق من ملف المدرس. تم منع إنشاء الامتحان لحماية بيانات المدرسة."
        : "Teacher profile verification failed. Exam creation has been blocked."
    );
    return;
  }
  ...
  if (!examTitle.trim()) {
    alert(
      lang === "ar"
        ? "يرجى إدخال عنوان الامتحان الشامل!"
  ...
  const assignedSections =
    ...
    activeTeacher.assignedSections.length > 0
      ? activeTeacher.assignedSections
      : [];
  ...
  if (
    examSection !== "الكل" &&
    !assignedSections.includes(examSection as Section)
  ) {
    alert(
      lang === "ar"
  ...
  alert(
    lang === "ar"
      ? `تم نشر امتحان مادة (${teacherSubject}) للصف (${examGrade} - شعبة ${
          examSection === "الكل" ? "جميع الشعب" : examSection
        }) بنجاح، وإشعار الطالبات فوراً!`
  ...
  const handleBulkMarkAttendance = (
    ...
    const filteredStudents = students.filter(
      (s) =>
        ...
        (attendanceSection === "الكل" || s.section === attendanceSection),
    );
  ...
  const handleSaveAttendance = () => {
    // Security: fail closed if the authenticated teacher profile is unavailable.
    if (!activeTeacher) {
      alert(
        lang === "ar"
          ? "تعذر التحقق من ملف المدرس. تم منع تسجيل الحضور والغياب لحماية بيانات المدرسة."
          : "Teacher profile verification failed. Attendance recording has been blocked.",
      );
      return;
    }

    // Security: the selected grade must be explicitly assigned to this teacher.
    const assignedGrades =
      activeTeacher.assignedGrades &&
      activeTeacher.assignedGrades.length > 0
        ? activeTeacher.assignedGrades
    ...
    if (!assignedGrades.includes(attendanceGrade)) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية تسجيل الحضور للصف (${attendanceGrade}).`
          : `You are not assigned to grade (${attendanceGrade}).`,
      );
      return;
    }

    // Security: no assigned sections means no attendance permission.
    const assignedSections =
      activeTeacher.assignedSections &&
      activeTeacher.assignedSections.length > 0
        ? activeTeacher.assignedSections
    ...
    if (assignedSections.length === 0) {
      alert(
        lang === "ar"
          ? "لا توجد شعب دراسية مخولة لهذا المدرس. تم منع تسجيل الحضور."
          : "No sections are assigned to this teacher. Attendance recording has been blocked.",
      );
      return;
    }

    if (
      attendanceSection !== "الكل" &&
      !assignedSections.includes(attendanceSection as Section)
    ) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية تسجيل الحضور للشعبة (${attendanceSection}).`
          : `You are not assigned to section (${attendanceSection}).`,
      );
      return;
    }
    const filteredStudents = students.filter((s) => {
      if (s.gradeLevel !== attendanceGrade) {
        return false;
      }

      // "الكل" means all sections assigned to THIS teacher only.
      if (attendanceSection === "الكل") {
        return (
          !!s.section &&
          assignedSections.includes(s.section as Section)
        );
      }

      return s.section === attendanceSection;
    });

    if (filteredStudents.length === 0) {
      alert(
        lang === "ar"
          ? "لا توجد طالبات مقيدات في هذا الصف والشعبة حالياً!"
    ...
    logAttendance(records, {
      teacherEmail: activeTeacher.email || currentUser?.email || "",
      teacherName: activeTeacher.name,
      teacherId: activeTeacher.id,
    });
    ...
    const reportPayload: AttendanceDispatchReport = {
      ...
      studentDetails: filteredStudents.map((std) => ({
        ...
        name: std.name,
        section: std.section || "أ",
        status: (attendanceMap[std.id] || "حاضرة") as
          | "حاضرة"
          | "غائبة"
          | "متأخرة"
          | "مجازة",
  ...
  const handlePrintCurrentAttendance = () => {
    // Security: Print blocked because teacher profile verification failed.
    if (!activeTeacher) {
      alert(
        lang === "ar"
          ? "تعذر التحقق من ملف المدرس. تم منع إنشاء تقرير الحضور."
          : "Teacher profile verification failed. Attendance report has been blocked.",
      );
      return;
    }

    const printAssignedGrades =
      activeTeacher.assignedGrades &&
      activeTeacher.assignedGrades.length > 0
        ? activeTeacher.assignedGrades
        : [];

    if (!printAssignedGrades.includes(attendanceGrade)) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية إنشاء تقرير حضور للصف (${attendanceGrade}).`
          : `You are not assigned to grade (${attendanceGrade}).`,
      );
      return;
    }

    const printAssignedSections =
      activeTeacher.assignedSections &&
      activeTeacher.assignedSections.length > 0
        ? activeTeacher.assignedSections
        : [];

    if (printAssignedSections.length === 0) {
      alert(
        lang === "ar"
          ? "لا توجد شعب دراسية مخولة لهذا المدرس. تم منع إنشاء التقرير."
          : "No sections are assigned to this teacher. Report generation has been blocked.",
      );
      return;
    }
    ...
    if (
      attendanceSection !== "الكل" &&
      !printAssignedSections.includes(attendanceSection as Section)
    ) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية إنشاء تقرير حضور للشعبة (${attendanceSection}).`
          : `You are not assigned to section (${attendanceSection}).`,
      );
      return;
    }
    const filteredStudents = students.filter((s) => {
      ...
      // "الكل" means only sections assigned to this teacher.
      if (attendanceSection === "الكل") {
        return (
          !!s.section &&
          printAssignedSections.includes(s.section as Section)
        );
      }
  ...
  const handleUploadLecture = (e: React.FormEvent) => {
    e.preventDefault();
    // Security: Lecture upload blocked because teacher profile verification failed.
    if (!activeTeacher) {
      alert(
        lang === "ar"
          ? "تعذر التحقق من ملف المدرس. تم منع رفع المرجع التعليمي."
          : "Teacher profile verification failed. Lecture upload has been blocked.",
      );
      return;
    }

    // Security: the teacher must have an explicitly assigned subject.
    const authorizedLectureSubject = activeTeacher.subject?.trim();

    if (!authorizedLectureSubject) {
      alert(
        lang === "ar"
          ? "لا توجد مادة دراسية مخولة لهذا المدرس. تم منع رفع المرجع."
          : "No subject is assigned to this teacher. Lecture upload has been blocked.",
      );
      return;
    }

    if (lecSubject.trim() !== authorizedLectureSubject) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية رفع محتوى لمادة (${lecSubject}).`
          : `You are not authorized to upload content for subject (${lecSubject}).`,
      );
      return;
    }

    // Security: the selected grade must be explicitly assigned to this teacher.
    const authorizedLectureGrades =
      activeTeacher.assignedGrades &&
      activeTeacher.assignedGrades.length > 0
        ? activeTeacher.assignedGrades
        : [];

    if (!authorizedLectureGrades.includes(lecGrade)) {
      alert(
        lang === "ar"
          ? `لا تملك صلاحية رفع محتوى للصف (${lecGrade}).`
          : `You are not assigned to grade (${lecGrade}).`,
      );
      return;
    }
    if (!lecTitle) return;
    ...
    const teacherRoleTitle = getTeacherSubjectTitle(
      activeTeacher || currentUser,
    );
    const supervisorRoleLabel = getSupervisorLabel(
      activeTeacher || currentUser,
    );
    ...
    addLecture({
      ...
      description:
        lecDesc ||
        `ملف تعليمي في مادة ${lecSubject} تم نشره بواسطة ${teacherNameFromUser}`,
      fileSize: "4.8 MB",
      pageCount: 120,
  ...
  // Security guard:
  // A teacher dashboard must never render without a verified teacher profile.
  if (!currentUser) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center p-6">
        <div className="max-w-lg rounded-2xl border bg-white p-6 text-center shadow-sm">
          ...
          <h2 className="text-lg font-bold text-slate-900">يجب تسجيل الدخول</h2>

          <p className="mt-2 text-sm text-slate-600">
            لا توجد جلسة مستخدم صالحة لعرض لوحة المدرس.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex min-h-[50vh] items-center justify-center p-6">
        <div className="max-w-lg rounded-2xl border bg-white p-6 text-center shadow-sm">
          ...
          <h2 className="text-lg font-bold text-slate-900">
            تعذر التحقق من ملف المدرس
          </h2>

          <p className="mt-2 text-sm text-slate-600">
            تم تسجيل الدخول، ولكن لم يتم العثور على ملف تدريسي مطابق لهوية
            المستخدم.
          </p>

          <p className="mt-2 text-xs text-slate-500">
            تم منع عرض لوحة المدرس لحماية بيانات المدرسة.
          </p>
        </div>
      </div>
    );
        ...
        <div className="p-6 rounded-3xl bg-indigo-600 text-white flex flex-wrap items-center justify-between gap-4 shadow-lg shadow-indigo-600/15">
          <div>
            ...
            <h2 className="text-xl font-black mt-1 text-white">
              {activeTeacher.name} - قسم {activeTeacher.subject}
            </h2>
            <p className="text-xs text-indigo-100 mt-1">
              {lang === "ar"
                ? "نظام إدارة الاختبارات الرقمية، تسجيل الحضور، ورفع المحاضرات التفاعلية"
                : "Online exam portal, attendance tracking, & resources"}
            </p>
          </div>
...
{/* Timetable Tab */}
              ...
              <div className="p-6 rounded-3xl bg-slate-900 border border-indigo-500/30 shadow-xl space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <h3 className="text-base font-bold text-white flex items-center gap-2">
                      ...
                      <span>
                        {t.markAttendance} (رصد اليوم والتنبيه الفوري):
                      </span>
                    </h3>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {lang === "ar"
                        ? "رصد الحضور والغياب اليومي حسب الصف والشعبة مع إشعارات أولياء الأمور"
                        : "Daily attendance tracker by grade and section"}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    ...
                    <div className="flex items-center gap-1.5 bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-700">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setAttendanceGrade(e.target.value as GradeLevel)
                        }
                    ...
                    <div className="flex items-center gap-1.5 bg-teal-950/60 px-3.5 py-1.5 rounded-xl border border-teal-500/50 shadow-sm shadow-teal-500/10">
                      ...
                      >
                        ...
                        <option value="أ" className="bg-slate-900 text-white">
                          شعبة ( أ )
                        </option>
                        <option value="ب" className="bg-slate-900 text-white">
                          شعبة ( ب )
                        </option>
                        <option value="جـ" className="bg-slate-900 text-white">
                          شعبة ( جـ )
                        </option>
                        <option value="د" className="bg-slate-900 text-white">
                          شعبة ( د )
                        </option>
                      </select>
                    </div>
                      ...
                      return (
                        <div className="p-8 text-center rounded-2xl bg-slate-800/40 border border-slate-800 space-y-2">
                          <p className="text-sm font-bold text-slate-400">
                            لا توجد طالبات مسجلات في {attendanceGrade} -{" "}
                            {attendanceSection === "الكل"
                              ? "جميع الشعب"
                              : `شعبة (${attendanceSection})`}
                          </p>
                          <p className="text-xs text-slate-500">
                            يرجى اختيار شعبة أخرى أو تغيير الصف الدراسي.
                          </p>
                        </div>
                      );
                    }
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5">
                            {(
                              ["حاضرة", "غائبة", "متأخرة", "مجازة"] as const
                            ).map((st) => (
                              <button
                                key={st}
                                onClick={() =>
                ...
                <div className="pt-4 border-t border-slate-800 flex flex-wrap items-center justify-between gap-3">
                  <div className="text-xs text-slate-300 flex items-center gap-1.5 bg-slate-900/80 px-3 py-1.5 rounded-xl border border-indigo-500/20">
                    ...
                    <span>
                      🔒 تنبيه إلكتروني سري وفردي لكل ولي أمر خاص بابنته فقط
                      (دون الاطلاع على حضور بقية الطالبات)
                    </span>
                  </div>
                  ...
                  {timetable.map((slot) => (
                    <tr key={slot.id} className="hover:bg-slate-800/50">
                      ...
                      <td className="p-3 font-mono text-amber-300">
                        {slot.timeSlot} (الحصة {slot.period})
                      </td>
                      <td className="p-3 text-indigo-200">
                        {slot.gradeLevel}{" "}
                        {slot.section ? `(شعبة ${slot.section})` : ""}
                      </td>
            ...
            <div className="bg-slate-900 border border-emerald-500/50 rounded-3xl max-w-4xl w-full p-6 sm:p-8 space-y-6 shadow-2xl text-right text-slate-100 max-h-[90vh] overflow-y-auto my-8">
              ...
              <div className="flex items-start justify-between border-b border-slate-800 pb-4">
                <div className="space-y-1">
                  ...
                  <h2 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2.5">
                    ...
                    <span>
                      تقرير تثبيت الحضور اليومي وإشعار البريد الإلكتروني
                    </span>
                  </h2>
                  <p className="text-xs text-slate-400">
                    كشف رسمي موثق صادِر عن ثانوية ميسان للمتميزات بتاريخ{" "}
                    {dispatchReportModal.date} (
                    {dispatchReportModal.dispatchedAt})
                  </p>
                </div>
              ...
              <div className="p-5 rounded-2xl bg-slate-800/90 border border-indigo-500/30 space-y-4">
                <h3 className="text-sm font-bold text-indigo-300 flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    ...
                    <span>
                      تخصيص وتوجيه التنبيهات الفورية والبريد الإلكتروني:
                    </span>
                  </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-900 border border-indigo-500/40 space-y-1.5">
                    ...
                    <div className="text-xs font-bold text-white">
                      {dispatchReportModal.teacherName}
                    </div>
                    <div className="text-[10px] font-mono text-slate-300 truncate">
                      {dispatchReportModal.teacherEmail}
                    </div>
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-900 border border-teal-500/40 space-y-1.5">
                    ...
                    <div className="text-xs font-bold text-teal-200">
                      {dispatchReportModal.gradeLevel} -{" "}
                      {dispatchReportModal.section === "الكل"
                        ? "جميع الشعب"
                        : `شعبة (${dispatchReportModal.section})`}
                    </div>
                    <div className="text-[10px] font-bold text-slate-300">
                      ({dispatchReportModal.totalStudents}) طالبة مستهدفة
                    </div>
                    <div className="inline-flex items-center gap-1 text-[10px] text-emerald-400 font-bold bg-emerald-950/60 px-2 py-0.5 rounded-md border border-emerald-800">
                      <CheckCircle2 className="w-3 h-3" />
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-900 border border-amber-500/40 space-y-1.5">
                    ...
                    <div className="text-xs font-mono text-slate-200 truncate">
                      {dispatchReportModal.adminEmail}
                    </div>
                    <div className="text-[10px] text-amber-200">
                      كشف شامل للمراجعة الإدارية
                    </div>
                    <div className="inline-flex items-center gap-1 text-[10px] text-emerald-400 font-bold bg-emerald-950/60 px-2 py-0.5 rounded-md border border-emerald-800">
                      <CheckCircle2 className="w-3 h-3" />
                ...
                <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-700/80 text-[11px] text-slate-300 flex items-center gap-2">
                  ...
                  <span>
                    <strong>تأكيد الأمان والسرية:</strong> تم استثناء بقية أعضاء
                    الهيئة التدريسية، بقية الطالبات، بقية أولياء الأمور، والمشرف
                    التربوي نهائياً من استقبال هذه التنبيهات.
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-center">
                <div className="p-3 rounded-2xl bg-slate-800 border border-slate-700">
                  ...
                  <div className="text-xl font-black text-white">
                    {dispatchReportModal.totalStudents}
                  </div>
                </div>
                <div className="p-3 rounded-2xl bg-emerald-950/50 border border-emerald-800/80">
                  ...
                  <div className="text-xl font-black text-emerald-400">
                    {dispatchReportModal.stats.present}
                  </div>
                </div>
                <div className="p-3 rounded-2xl bg-rose-950/50 border border-rose-800/80">
                  ...
                  <div className="text-xl font-black text-rose-400">
                    {dispatchReportModal.stats.absent}
                  </div>
                </div>
                <div className="p-3 rounded-2xl bg-amber-950/50 border border-amber-800/80">
                  ...
                  <div className="text-xl font-black text-amber-400">
                    {dispatchReportModal.stats.late}
                  </div>
                </div>
                <div className="p-3 rounded-2xl bg-indigo-950/50 border border-indigo-800/80 col-span-2 sm:col-span-1">
                  ...
                  <div className="text-xl font-black text-indigo-300">
                    {dispatchReportModal.stats.excused}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-300">
                  سجل طالبات الشعبة والبريد الإلكتروني الموجه لكل ولي أمر:
                </h4>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const success =
                          await exportAttendanceDispatchReportAsPdf(
                            dispatchReportModal,
                          );
                        if (!success) {
                          const reportEl = document.getElementById(
                            "teacher-attendance-printable-report",
                          );
                          if (reportEl) {
        ...
        >
          ...
          <div className="border-b-2 border-slate-900 pb-4 mb-6 flex items-center justify-between">
            <div className="text-right space-y-1">
              ...
              <p className="text-xs font-bold text-slate-800">
                المديرية العامة لتربية محافظة ميسان
              </p>
              <p className="text-sm font-black text-slate-950">
                ثانوية ميسان للمتميزات
              </p>
              <p className="text-[10px] text-slate-600 font-mono">
                نظام الحضور الرقمي المعتمد
              </p>
            </div>
            ...
            <div className="text-center space-y-1">
              ...
              <h1 className="text-base font-black text-slate-950 mt-1">
                سجل ورصد الحضور والغياب اليومي الرسمي
              </h1>
            </div>

            <div className="text-left text-xs space-y-1 font-mono text-slate-800">
              <p>
                <span className="font-bold font-sans">التاريخ:</span>{" "}
                {dispatchReportModal.date}
              </p>
              <p>
                <span className="font-bold font-sans">توقيت التوثيق:</span>{" "}
                {dispatchReportModal.dispatchedAt}
              </p>
              <p>
                <span className="font-bold font-sans">الصف:</span>{" "}
                {dispatchReportModal.gradeLevel}
              </p>
              <p>
                <span className="font-bold font-sans">الشعبة:</span> (
                {dispatchReportModal.section === "الكل"
                  ? "جميع الشعب"
                  : dispatchReportModal.section}
                )
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-3 p-3 border border-slate-900 rounded-lg mb-4 text-xs bg-slate-50">
            ...
            <div>
              ...
              <span className="font-black text-slate-950">
                {dispatchReportModal.totalStudents} طالبة
              </span>
            </div>
          </div>
          ...
          <table className="w-full text-right text-xs border-collapse border border-slate-900 mb-6">
            <thead>
              <tr className="bg-slate-200 text-slate-950 font-black border-b border-slate-900">
                <th className="p-2 border border-slate-900 text-center w-8">
                  ت
                </th>
                <th className="p-2 border border-slate-900">
                  اسم الطالبة الثلاثي
                </th>
                <th className="p-2 border border-slate-900 text-center">
                  الشعبة
                </th>
                <th className="p-2 border border-slate-900 text-center">
                  حالة الحضور
                    ...
                    <span
                      className={
                        std.status === "حاضرة"
                          ? "text-slate-900"
                          : std.status === "غائبة"
                            ? "text-rose-900 font-black underline"
                            : std.status === "متأخرة"
      ...
      <UploadPdfModal
        ...
        defaultSubject={activeTeacher.subject}
        defaultGrade={activeTeacher.assignedGrades?.[0]}
        defaultTeacherName={activeTeacher.name}
        verifiedTeacherId={activeTeacher.authUid || activeTeacher.id}
        verifiedTeacherRole="teacher"
        authorizedSubject={activeTeacher.subject}
        authorizedGrades={activeTeacher.assignedGrades || []}
      />
